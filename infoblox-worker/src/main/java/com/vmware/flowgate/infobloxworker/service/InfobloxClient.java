...
public class InfobloxClient extends RestClientBase {
   ...
   public List<InfoBloxResult> queryHostNamesByIP(String ip) {
      ...
      List<InfoBloxResult> results = jsonResultForQueryHostNames.getResult().stream().map(InfoBloxResult::build).collect(Collectors.toList());
