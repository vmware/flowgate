   ...
   public void getMetricDatas() {
      ...
      for(RealTimeData data : metricDatas) {
         if("assetid1611766210404".equals(data.getId())) {
            ...
            TestCase.assertEquals(5, values.size());
         ...
         }else if("assetid1611334210404".equals(data.getId())) {
            //Temperature metric
            List<ValueUnit> values = data.getValues();
            TestCase.assertEquals(3, values.size());
            for(ValueUnit value : values) {
               switch (value.getKey()) {
         ...
         }else if("assetid1609595105147".equals(data.getId())) {
            List<ValueUnit> values = data.getValues();
